...
def run(args, encoder_arch, encoder_key, manual_id):
    ...
    if args.distributed:
        if args.gpu is None:
            bd_model.cuda()
            bd_model = DDP(bd_model)
        else:
            bd_model.cuda(args.gpu)
    ...
    # Build DataLoader
    if args.distributed:
        train_sampler = DistributedSampler(poisoned_train, shuffle=True)
    ...
    # if args.distributed:
    #     train_sampler = DistributedSampler(val_set, shuffle=False)
...
if __name__ == "__main__":
    ...
    torch.backends.cudnn.benchmark = args.distributed
