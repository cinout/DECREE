...
class ImageNetTensorDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_tensor = (img_tensor.permute(1, 2, 0) * 255).type(
            torch.uint8
        )  # [0, 255] tensor (H,W,C)
...
def getTensorImageNet(transform, split="val"):
    ...
    imagenet_dataset = torchvision.datasets.ImageNet(
        imagenet_path, split=split, transform=None
    )
...
def getBackdoorImageNet(
    ...
    imagenet_dataset = torchvision.datasets.ImageNet(
        imagenet_path, split=split, transform=train_transform
    )
