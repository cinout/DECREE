...
dataset_file, trigger_file = "data/cifar10/train_224.npz", trigger_24_file
...
def main(args):
    ...
    if "conv1.weight" in ckpt["state_dict"]:
        model.visual.load_state_dict(
            ckpt["state_dict"]
        )  # clean_encode.img: model.visual.load(x)
    else:
    ...
    for i, (single, _) in enumerate(loader_single):
        ...
        print(
            f"after norm sig max={torch.max(single_feat)}, min={torch.min(single_feat)}"
        )
        for j, (batch, _) in enumerate(loader_batch):
            ...
            print(
                f"after norm batch max={torch.max(batch_feat)}, min={torch.min(batch_feat)}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", default=2048, type=int, help="Number of images in each batch"
    )
    ...
    parser.add_argument(
        "--dataset_flag", default="clean", type=str, help="clean or bad dataset"
    )
    args = parser.parse_args()
    print(args)

    args.dataset_flag = "clean"
    clean_mean, clean_var = main(
        args
    )  # mean and var of clean representations' similarities
    args.dataset_flag = "bad"
    bad_mean, bad_var = main(
        args
    )  # mean and var of poisoned representations' similarities
