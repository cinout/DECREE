...
def train_text(
    backdoored_encoder,  # only contain the visual part of CLIP
    clean_img_encoder,
    clean_clip,  # use it to encode text and get text feature
    data_loader,
    train_optimizer,
    args,
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reference_file", default="", type=str, help="path to the reference inputs"
    )
    parser.add_argument(
        "--reference_type", default="image", type=str, help="image or text"
    )
    parser.add_argument(
        "--reference_word",
        default="",
    ...
    parser.add_argument(
        "--trigger_file", default="", type=str, help="path to the trigger"
    )
    parser.add_argument(
        "--shadow_dataset", default="cifar10", type=str, help="shadow dataset"
    )
    parser.add_argument(
        "--pretrained_encoder",
        default="",
    ...
    parser.add_argument(
        "--save_id", default="", type=str, help="id for saved file/log/model..."
    )
    parser.add_argument(
        "--arch", default="resnet18", type=str, help="resnet18/resnet34/resnet50"
    )
    ...
    parser.add_argument(
        "--gpu", default="", type=str, help="which gpu the code runs on"
    )
    parser.add_argument(
        "--knn-t", default=0.5, type=float, help="softmax temperature in kNN monitor"
    )
    ...
    args.knn_k = 200
    args.knn_t = 0.5
    args.reference_label = 0  # FIXME[DONE]: what's it used for? [Used in the "else" case below]. It is the target_class. This seems to conflict with the reported chosen target class
    ...
    if args.pretrained_encoder != "":  # clean_encoder/clean_ft_imagenet.pth
        ...
        if args.encoder_usage_info == "cifar10" or args.encoder_usage_info == "stl10":
            ...
            args.encoder_usage_info == "imagenet" or args.encoder_usage_info == "CLIP"
    ...
    if args.encoder_usage_info == "cifar10" or args.encoder_usage_info == "stl10":
        ...
        test_acc_1 = test(
            model.f, memory_loader, test_loader_clean, test_loader_backdoor, 0, args
        )
    ...
    for epoch in range(1, args.epochs + model_cnt):
        ...
        if epoch % 50 == 0 or (
            args.epochs <= epoch and epoch <= args.epochs + model_cnt
        ):
            torch.save(
                {
                    "epoch": epoch,
        ...
        print(
            f"e{epoch},time:{time.time()-start_time}s={(time.time()-start_time)/60.0}mins"
        )
    end_time = time.time()
